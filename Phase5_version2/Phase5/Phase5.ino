...
void challenge2_state() {
  ...
  if (signal_300 || debounce(digitalRead(sensors[0]), 0)) {
    ...
    start_time = millis();
    while (millis() - start_time < 725) { // turn left
      L_turn();
    }
    for (int i = 0; i < 3; i++) {
      brake();
      delay(300);
      start_time = millis();
      while (millis() - start_time < 540) { // turn right 3 x
        R_turn();
      }
    }
    start_time = millis();
